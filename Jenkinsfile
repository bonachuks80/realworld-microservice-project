...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/bonachuks80/realworld-microservice-project.git'
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.23.88.23 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.220.241.136:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#bce-devsecops-cicd-alerts', //update and provide your channel name
